...
def drow_detection_to_kitti_string(dets_xy, dets_cls, occluded):
    ...
    if occluded is None:
        occluded = np.zeros(len(dets_xy), dtype=int)
...
def kitti_string_to_drow_detection(s):
    ...
    occluded = np.array(occluded, dtype=int)
...
def get_precision_recall_one_hot(
    ...
    if dist_bins is not None:
        gt_hist = np.zeros(len(dist_bins), dtype=int)
        tp_hist = np.zeros(len(dist_bins), dtype=int)
        fp_hist = np.zeros(len(dist_bins), dtype=int)
