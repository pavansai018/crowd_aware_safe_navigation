...
class SpatialEdgeSelfAttn(nn.Module):
    ...
    def __init__(self, args, config):
        ...
        # pred 5 steps + no disp + conformity scores for each prediction: 12 + 5 = 17
...
class selfAttn_merge_SRNN(nn.Module):
    ...
    def forward(self, inputs, rnn_hxs, masks, infer=False):
        ...
        if self.args.sort_humans:
            ...
            # if not sort_humans, conformity_scores should also be changed
            raise NotImplementedError
        ...
        if self.config.policy.aci_input:
            spatial_edges = torch.cat((spatial_edges, conformity_scores), dim=-1)        # attention modules
        if self.args.sort_humans:
